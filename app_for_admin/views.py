...
def chartData(request):
    if request.is_ajax() and request.method == "GET":
        if request.user.is_staff:
            data = BillDetail.objects.filter(bill__date__year=request.GET.get('year'), bill__date__month=request.GET.get('month')).values('product').order_by('product').annotate(total=Sum('quantity')).order_by('-total')[:20]
...
def exportUsersXls(request):
    ...
    if request.user.is_staff:
        ...
        columns = ['Bill id', 'Date', 'Product', 'Size', 'Quantity', 'Unit price']
        ...
        rows = []
        data = BillDetail.objects.filter(bill__date__month=int(monthYear[1]), bill__date__year=int(monthYear[0])).values_list('bill__id','bill__date__day','bill__date__month', 'bill__date__year','product__product', 'product__size', 'quantity', 'unit_price')
        for d in data:
